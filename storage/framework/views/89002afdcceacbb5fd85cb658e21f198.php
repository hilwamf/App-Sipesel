...
<?php $__env->startSection('title','Berita Pajak - SIPESEL'); ?>
<?php $__env->startSection('page-title','Berita & Info Pajak'); ?>
<?php $__env->startSection('page-sub','Kelola informasi dan edukasi pajak untuk pedagang'); ?>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        <div class="relative h-36 bg-gray-100 overflow-hidden flex-shrink-0">
            ...
            <?php endif; ?>
            <span class="absolute top-2 right-2 px-2.5 py-1 rounded-full text-[10px] font-bold <?php echo e($b->status==='aktif'?'bg-green-500 text-white':'bg-gray-400 text-white'); ?>"><?php echo e($b->status==='aktif'?'● Aktif':'○ Nonaktif'); ?></span>
        </div>
        <div class="px-4 py-3 flex-1">
            <p class="text-gray-400 text-[10px] mb-1"><?php echo e($b->created_at->format('d M Y')); ?></p>
            <h3 class="font-bold text-gray-800 text-sm mb-1.5 leading-snug"><?php echo e($b->judul); ?></h3>
            <p class="text-gray-500 text-xs line-clamp-2"><?php echo e($b->isi); ?></p>
        </div>
        <div class="px-4 py-3 border-t border-gray-50 flex gap-2">
            <button onclick='showEditModal(<?php echo json_encode($b, 15, 512) ?>)' class="flex-1 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 font-semibold rounded-xl text-xs">Edit</button>
            <form method="POST" action="<?php echo e(route('admin.toggle-berita')); ?>" class="flex-1"><?php echo csrf_field(); ?><input type="hidden" name="id" value="<?php echo e($b->id); ?>"><button class="w-full py-2 <?php echo e($b->status==='aktif'?'bg-yellow-50 hover:bg-yellow-100 text-yellow-700':'bg-green-50 hover:bg-green-100 text-green-700'); ?> font-semibold rounded-xl text-xs"><?php echo e($b->status==='aktif'?'Nonaktifkan':'Aktifkan'); ?></button></form>
            <form method="POST" action="<?php echo e(route('admin.hapus-berita')); ?>" onsubmit="return confirm('Hapus?')"><?php echo csrf_field(); ?><input type="hidden" name="id" value="<?php echo e($b->id); ?>"><button class="py-2 px-3 bg-red-50 hover:bg-red-100 text-red-600 font-semibold rounded-xl text-xs">Hapus</button></form>
        </div>
    </div>
...
<div id="modalTambah" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="<?php echo e(route('admin.tambah-berita')); ?>" enctype="multipart/form-data" class="space-y-4"><?php echo csrf_field(); ?>
...
<div id="modalEdit" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="<?php echo e(route('admin.edit-berita')); ?>" enctype="multipart/form-data" class="space-y-4"><?php echo csrf_field(); ?>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.sidebar-admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Sipesel\resources\views/admin/berita.blade.php ENDPATH**/ ?>