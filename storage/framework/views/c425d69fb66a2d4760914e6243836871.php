<?php $__env->startSection('title','Manajemen Pengguna - SIPESEL'); ?>
<?php $__env->startSection('page-title','Manajemen Pengguna'); ?>
<?php $__env->startSection('page-sub','Kelola data pedagang, pengawas, dan admin'); ?>

<?php $__env->startSection('content'); ?>

<?php if(session('success')): ?><div class="bg-green-50 border border-green-200 text-green-800 rounded-xl px-5 py-3 mb-5 flex items-center gap-2 text-sm"><svg class="w-4 h-4 text-green-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg><?php echo e(session('success')); ?></div><?php endif; ?>
<?php if(session('error')): ?><div class="bg-red-50 border border-red-200 text-red-700 rounded-xl px-5 py-3 mb-5 text-sm"><?php echo e(session('error')); ?></div><?php endif; ?>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
    <div class="flex flex-wrap gap-3 items-end justify-between">
        <form method="GET" class="flex flex-wrap gap-3 flex-1">
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama, username, email..."
            ...
            <select name="filter" onchange="this.form.submit()" class="px-4 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500">
                ...
                <option value="pedagang" <?php echo e(request('filter')=='pedagang'?'selected':''); ?>>Pedagang</option>
                <option value="pengawas" <?php echo e(request('filter')=='pengawas'?'selected':''); ?>>Pengawas</option>
        ...
        <button onclick="document.getElementById('modalAdd').classList.remove('hidden')" class="px-5 py-2.5 bg-green-700 hover:bg-green-800 text-white font-semibold rounded-xl text-sm flex items-center gap-2">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-400 bg-gray-100 px-3 py-1 rounded-full"><?php echo e($users->count()); ?> pengguna</span>
    </div>
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-sm flex-shrink-0"><?php echo e(strtoupper(substr($u->username,0,1))); ?></div>
                            <div>
                                <p class="font-semibold text-gray-800 text-sm"><?php echo e($u->nama); ?></p>
                                <p class="text-gray-400 text-xs"><?php echo e($u->username); ?> · <?php echo e($u->nomor_hp); ?></p>
                            </div>
                        </div>
                    </td>
                    <td class="px-5 py-4 text-sm text-gray-600"><?php echo e($u->email); ?></td>
                    <td class="px-5 py-4">
                        <span class="px-3 py-1 rounded-full text-xs font-semibold <?php echo e($u->role==='admin'?'bg-red-100 text-red-700':($u->role==='pengawas'?'bg-blue-100 text-blue-700':'bg-green-100 text-green-700')); ?>"><?php echo e(ucfirst($u->role)); ?></span>
                    </td>
                    <td class="px-5 py-4">
                        <?php if($u->no_kios): ?><span class="px-2 py-1 bg-blue-50 text-blue-700 font-bold text-xs rounded-lg"><?php echo e($u->no_kios); ?></span>
                        <?php else: ?><span class="text-gray-300">—</span><?php endif; ?>
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex gap-2">
                            <button onclick='showEditModal(<?php echo json_encode($u, 15, 512) ?>)' class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-semibold rounded-lg text-xs">Edit</button>
                            <form method="POST" action="<?php echo e(route('admin.reset-pw')); ?>" onsubmit="return confirm('Reset password ke password123?')">
                                <?php echo csrf_field(); ?><input type="hidden" name="id" value="<?php echo e($u->id_user); ?>">
                            ...
                            <?php if($u->id_user !== auth()->id()): ?>
                            <form method="POST" action="<?php echo e(route('admin.delete-user')); ?>" onsubmit="return confirm('Hapus user ini?')">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?><input type="hidden" name="id" value="<?php echo e($u->id_user); ?>">
...
<div id="modalAdd" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="<?php echo e(route('admin.add-user')); ?>" class="space-y-3"><?php echo csrf_field(); ?>
...
<div id="modalEdit" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="<?php echo e(route('admin.update-user')); ?>" class="space-y-3"><?php echo csrf_field(); ?>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.sidebar-admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Sipesel\resources\views/admin/users.blade.php ENDPATH**/ ?>