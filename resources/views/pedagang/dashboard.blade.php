...
<div class="max-w-6xl mx-auto px-6 py-12 w-full">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-10">
        ...
        {{-- KIRI: Foto Profil + Sapaan (compact & rapi) --}}
        <div class="flex flex-col items-center justify-center">
            ...
            <div class="glass-card rounded-3xl p-8 text-center w-full max-w-sm float-anim">
                    ...
                    <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-yellow-400 shadow-xl mx-auto">
                        ...
                        @else
                            <div class="w-full h-full bg-green-600 flex items-center justify-center">
                                <span class="text-4xl font-black text-yellow-400" style="font-family:'Montserrat',sans-serif;">{{ strtoupper(substr($user->nama,0,1)) }}</span>
                            </div>
                        @endif
                    </div>
                ...
                <h2 class="text-white font-black text-2xl mb-1" style="font-family:'Montserrat',sans-serif;">{{ $user->username }}</h2>
                <p class="text-white/50 text-xs mb-5">Kios <span class="text-yellow-400 font-bold">{{ $user->no_kios ?? '-' }}</span> · Pasar Wadungasri</p>
                ...
                <div class="border-t border-white/10 pt-4 space-y-2">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="text-white font-bold text-xs">{{ $jatuhTempoStr }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="text-white font-bold text-xs">
                            {{ $totalBulanIni > 0 ? 'Rp '.number_format($totalBulanIni,0,',','.') : '—' }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="text-white">
            ...
            <div class="glass-card rounded-2xl px-5 py-4 mb-6 grid grid-cols-3 gap-3">
                <div class="text-center">
                    ...
                    <p class="text-white font-black text-lg" style="font-family:'Montserrat',sans-serif;">{{ $user->no_kios ?? '-' }}</p>
                </div>
                <div class="text-center border-x border-white/10">
                    ...
                    <p class="text-white font-black text-sm">
                        {{ $totalBulanIni > 0 ? 'Rp '.number_format($totalBulanIni/1000,0,',','.').'K' : '-' }}
                    </p>
                </div>
                <div class="text-center">
                    <p class="text-green-300 text-[10px] uppercase tracking-wider mb-1">Status</p>
                    <p class="text-[11px] font-bold {{ $sudahLewat ? 'text-red-300' : 'text-green-300' }}">
                        {{ $sudahLewat ? '⚠ Jatuh Tempo' : ($jatuhTempoStr ? '✓ Aktif' : '— Belum Ada') }}
                    </p>
                </div>
            </div>

            <div class="flex gap-3 flex-wrap">
                <a href="{{ route('pedagang.pembayaran') }}" class="px-7 py-3.5 bg-yellow-400 hover:bg-yellow-300 text-green-900 font-bold rounded-full transition-all hover:-translate-y-0.5 shadow-lg shadow-yellow-400/30 text-sm flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                    Bayar Pajak
                </a>
                <a href="{{ route('pedagang.riwayat') }}" class="px-7 py-3.5 glass-card hover:bg-white/20 text-white font-semibold rounded-full transition-all text-sm flex items-center gap-2">
    ...
    {{-- BERITA CAROUSEL + RIWAYAT TRANSAKSI SIDE BY SIDE --}}
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">

        {{-- KIRI: Carousel Berita --}}
        <div>
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-green-300 text-xs">{{ $beritaList->count() }} berita</span>
            </div>
            <div class="relative rounded-3xl overflow-hidden" style="height:340px;">
                <div id="beritaTrack" class="flex h-full transition-transform duration-700 ease-in-out">
                    ...
                    <div class="min-w-full h-full relative flex-shrink-0">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-5">
                            ...
                            <h3 class="text-white font-bold text-sm mb-1.5 leading-snug">{{ $b->judul }}</h3>
                            <p class="text-green-200 text-xs leading-relaxed line-clamp-2">{{ $b->isi }}</p>
                            <p class="text-green-400/60 text-[10px] mt-2">{{ $b->created_at->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="absolute bottom-3 right-4 flex gap-1.5 z-10" id="beritaDots">
                    ...
                    <button onclick="goBerita({{ $i }})" class="berita-dot transition-all duration-300 rounded-full bg-white/40 h-2 {{ $i===0?'w-6 bg-white/90':'w-2' }}"></button>
                    @endforeach
                </div>
                @endif
            </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('pedagang.riwayat') }}" class="text-yellow-400 text-xs font-semibold hover:text-yellow-300 transition">Lihat Semua →</a>
            </div>
            <div class="glass-card rounded-3xl overflow-hidden" style="height:340px;">
                ...
                <div class="h-full overflow-y-auto divide-y divide-white/10">
                    ...
                    <div class="px-5 py-4 flex items-center gap-4 hover:bg-white/5 transition-all">
                        <div class="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-xl flex-shrink-0">{{ $icon }}</div>
                        <div class="flex-1 min-w-0">
                            <p class="text-white font-semibold text-sm">Pajak {{ $trx->jenis_pajak }}</p>
                            <p class="text-green-300 text-xs mt-0.5">{{ $trx->tanggal->format('d M Y, H:i') }}</p>
                        </div>
                        <div class="text-right flex-shrink-0">
                            <p class="text-white font-bold text-sm">Rp {{ number_format($trx->nominal,0,',','.') }}</p>
                            <span class="inline-block px-2 py-0.5 {{ $badge['bg'] }} {{ $badge['text'] }} rounded-full text-[10px] font-semibold mt-1">{{ $badge['label'] }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="h-full flex items-center justify-center text-center p-8">
                    <div>
                        ...
                        <a href="{{ route('pedagang.pembayaran') }}" class="inline-block mt-3 px-5 py-2 bg-yellow-400 text-green-900 font-semibold rounded-full text-xs hover:bg-yellow-300 transition">Bayar Sekarang</a>
                    </div>
                </div>
                @endif
            </div>
...
(function() {
    ...
    @if(isset($jatuhTempoDatetime) && $jatuhTempoDatetime)
    const target = new Date('{{ $jatuhTempoDatetime }}');
    function update() {
        const now = new Date();
