    ...
    <div class="border-b border-green-600/50 py-2 px-6">
        <div class="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('profil.edit') }}" class="text-green-100 text-sm font-medium hidden sm:block hover:text-yellow-300 transition">{{ $user->username }}</a>
            </div>
        </div>
    </div>
...
{{-- Panel notif FIXED position supaya tidak terpotong --}}
@if(isset($showNotif) && $showNotif)
<div id="notifPanel" style="display:none;position:fixed;top:80px;right:24px;z-index:99999;width:320px;" class="bg-green-900 border border-white/20 rounded-xl shadow-2xl overflow-hidden">
    <div class="px-4 py-3 border-b border-white/10 flex items-center justify-between bg-green-800">
        <p class="font-semibold text-sm text-white flex items-center gap-2">
            ...
            Notifikasi
            @if(isset($unread) && $unread > 0)
            <span class="px-2 py-0.5 bg-red-500 text-white text-xs rounded-full">{{ $unread }}</span>
            @endif
        </p>
    ...
    <div style="max-height:280px;overflow-y:auto;">
        ...
        <div class="px-4 py-3 border-b border-white/5 hover:bg-white/5 transition-all {{ !$n->dibaca ? 'bg-yellow-500/10 border-l-2 border-l-yellow-400' : '' }}">
...
(function() {
    ...
    btn.addEventListener('click', function(e) {
        ...
        if (!isOpen && badge) {
            // Mark as read
            fetch('{{ route("pedagang.mark-notif-read") }}', {
                method: 'POST',
                headers: {
