...
@section('title', 'Login SIPESEL')
@section('body-attrs', 'style="background-image:url(\''.asset('images/kios-pasar.jpg.jpeg').'\');" class="min-h-screen flex items-center justify-center bg-cover bg-center"')
...
<div class="relative z-10 bg-white/10 backdrop-blur-lg p-8 md:p-10 rounded-2xl w-[90%] max-w-sm text-white text-center shadow-2xl">
    ...
    @endif
    @if(session('error'))
        <div class="bg-red-500/80 text-white text-sm p-3 rounded-lg mb-4">{{ session('error') }}</div>
    @endif
    @if($errors->any())
